function createMapaFromImage(maze){var mazeCv=document.createElement("canvas");
...
for(line=0;
line<maxline;
line++){map[line]=new Array(maxcol);
for(col=0;
col<maxcol;
col++){index=(col+line*(imagem.width))*4;
rgba[0]=imagem.data[index];
rgba[1]=imagem.data[index+1];
rgba[2]=imagem.data[index+2];
rgba[3]=imagem.data[index+3];
if(rgba[0]===0&&rgba[1]===0&&rgba[2]===0){tileValue=FECHADO;
}else{if(rgba[0]===255&&rgba[1]===255&&rgba[2]===255){tileValue=ABERTO;
}else{tileValue=ABERTO;
}}map[line][col]=tileValue;
}}return map;
}var cleanmapa=function(){var maxline=32;
var maxcol=42;
var line,col;
var map=new Array(maxline);
for(line=0;
...
for(col=0;
col<maxcol;
col++){if((line===0||line===(maxline-1))||(col===0||col===(maxcol-1))){map[line][col]=-1;
}else{map[line][col]=0;
}}}return map;
}();
var dirtmapa,cmapa,dirtmapa2;
function loadMaps(){var maze1=new Image();
